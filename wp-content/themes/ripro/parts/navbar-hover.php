    ...
    <div class="header-box">
      ...
      <div class="header-center">
        <div class="md-l">
          ...
          <a href="<?php echo esc_url(home_url('/user?action=charge'));?>" class="pay-credit" >充值</a>
        </div>
        <div class="md-r">
        <div class="md-t">
        ...
        <?php 
          if ($CaoUser->vip_status()) {
              echo '<p>到期时间：'.$CaoUser->vip_end_time().'</p><a href="'.esc_url(home_url('/user?action=vip')).'" class="pay-vip">续费</a>';
          }else{
              echo '<p>'._cao('navbar_newhover_text1').'</p><a href="'.esc_url(home_url('/user?action=vip')).'" class="pay-vip">开通</a>';
          }
        ?>
      </div>
      <div class="md-b">
        ...
        <?php if (is_boosvip_status($current_user->ID)) {
            ...
            echo '<p>'._cao('navbar_newhover_text2').'</p><a href="'.esc_url(home_url('/user?action=vip')).'" class="pay-vip">升级</a>';
        }?>
      ...
      <div class="header-bottom">
        <ul>
          <li><a href="<?php echo esc_url(home_url('/user?action=myfav'));?>"><i class="ico_1"></i>我的收藏</a></li>
          ...
          <?php else : ?>
          <li><a href="<?php echo esc_url(home_url('/user?action=mypost'));?>"><i class="ico_2"></i>我的文章</a></li>
          <?php endif; ?>
          <li><a href="<?php echo esc_url(home_url('/user?action=password'));?>"><i class="ico_3"></i>安全中心</a></li>
          <li><a href="<?php echo esc_url(home_url('/user?action=mypay'));?>"><i class="ico_4"></i>我的订单</a></li>
          <?php if(in_array( 'administrator', $current_user->roles )): ?>
            <li><a target="_blank" href="<?php echo home_url('/wp-admin/') ?>"><i class="ico_5"></i>后台管理</a></li>
          <?php else : ?>
            <li><a href="<?php echo esc_url(home_url('/user?action=write'));?>"><i class="ico_5"></i>我要投稿</a></li>
          <?php endif; ?>
        </ul>
      </div>
    <?php endif;?>
